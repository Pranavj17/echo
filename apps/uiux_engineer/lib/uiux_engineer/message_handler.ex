defmodule UiuxEngineer.MessageHandler do
  @moduledoc """
  Handles incoming messages from other agents via Redis pub/sub.

  Subscribes to:
  - messages:uiux_engineer (direct messages)
  ...
  @impl true
  def init(_opts) do
    Logger.info("UIUX_ENGINEER Message Handler started")

    # Subscribe to UIUX_ENGINEER messages
    {:ok, _} = MessageBus.subscribe_to_role(:uiux_engineer)
  ...
  defp handle_message("messages:uiux_engineer", message, state) do
    Logger.info("UIUX_ENGINEER received message: #{message["subject"]} from #{message["from"]}")
  ...
  defp handle_message("messages:all", message, state) do
    Logger.info("UIUX_ENGINEER received broadcast: #{message["subject"]}")

    message_id = message["id"] || "unknown"

    if MapSet.member?(state.recent_broadcasts, message_id) do
      Logger.debug("UIUX_ENGINEER already evaluated broadcast #{message_id}, skipping")
      state
    else
      case ParticipationEvaluator.should_participate?(:uiux_engineer, message) do
        {:yes, confidence, type} ->
          Logger.info("UIUX_ENGINEER participating as #{type} (confidence: #{confidence})")
          participate_in_task(message, type)

        {:no, reason} ->
          Logger.debug("UIUX_ENGINEER declining participation: #{reason}")

        {:defer, _seconds} ->
          Logger.debug("UIUX_ENGINEER evaluating with LLM (async)...")
      end
  ...
  defp handle_message("messages:leadership", message, state) do
    Logger.info("UIUX_ENGINEER received leadership message: #{message["subject"]}")
  ...
  defp handle_escalation(message) do
    ...
    # TODO: Handle escalations requiring UIUX_ENGINEER attention
  end
  ...
  defp handle_escalated_decision(event) do
    # Log escalated decision for UIUX_ENGINEER review
    Logger.warning("""
    ESCALATED DECISION REQUIRES UIUX_ENGINEER REVIEW
  ...
  defp participate_in_task(message, participation_type) do
    ...
    Logger.info("UIUX_ENGINEER participating in task: #{subject} (as #{participation_type})")

    response_content = case participation_type do
      :lead -> "I'll lead this design initiative. My UI/UX expertise will ensure excellent user experience."
      :assist -> "I can assist with this. Let me know how I can contribute from a design perspective."
      :observe -> "I'll monitor this work to ensure design consistency and user experience quality."
    end

    MessageBus.publish_message("uiux_engineer", from, :response, "Re: #{subject}", %{
      content: response_content,
      participation_type: participation_type,
      timestamp: DateTime.utc_now() |> DateTime.to_iso8601()
    })

    MessageBus.store_message_in_db("uiux_engineer", from, "response", "Re: #{subject}", %{
      content: response_content,
      participation_type: participation_type
    })

    Logger.info("✓ UIUX_ENGINEER participation signal sent")
  end
end

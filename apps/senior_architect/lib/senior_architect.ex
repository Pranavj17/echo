defmodule SeniorArchitect do
  @moduledoc """
  SENIOR_ARCHITECT (Chief Technology Officer) MCP Server.

  The SENIOR_ARCHITECT is responsible for technology strategy, infrastructure architecture,
  ...
  def tools do
    [
      %{
        ...
        inputSchema: %{
          ...
          properties: %{
            ...
            rationale: %{
              type: "string",
              description: "SENIOR_ARCHITECT's rationale for the approval/rejection"
            },
            approved: %{
              type: "boolean",
      ...
      %{
        ...
        description: "Escalate a decision to CEO when it exceeds SENIOR_ARCHITECT authority",
        inputSchema: %{
          type: "object",
          properties: %{
            decision_id: %{
            ...
            recommendation: %{
              type: "string",
              description: "SENIOR_ARCHITECT's recommendation to CEO (optional)"
            }
          },
  ...
  def execute_tool("approve_technical_proposal", args) do
    with {:ok, proposal_id} <- validate_required_string(args, "proposal_id"),
      ...
      Decision: #{if approved, do: "Approved", else: "Rejected"} by SENIOR_ARCHITECT
      Rationale: #{rationale}
  ...
  def execute_tool("allocate_engineering_budget", args) do
    with {:ok, recipient} <- validate_required_string(args, "recipient"),
      ...
      MessageBus.publish_message(
        ...
        :ceo,
        :notification,
        "SENIOR_ARCHITECT Budget Allocation: $#{format_number(amount)}",
        allocation
      )
      ...
      {:error, :budget_limit_exceeded} ->
        {:error, "Budget amount exceeds SENIOR_ARCHITECT autonomous authority ($500,000). Requires CEO approval."}
  ...
  def execute_tool("escalate_to_ceo", args) do
    with {:ok, decision_id} <- validate_required_string(args, "decision_id"),
      ...
      Urgency: #{urgency}
      Reason: #{reason}
      SENIOR_ARCHITECT Recommendation: #{args["recommendation"] || "None provided"}
    ...
    with {:ok, query_type} <- validate_required_string(args, "query_type"),
      ...
      result = case query_type do
        "decision_analysis" ->
          ...
          DecisionHelper.analyze_decision(:senior_architect, decision_context)
        ...
        "option_evaluation" ->
          ...
          DecisionHelper.evaluate_options(:senior_architect, evaluation_context)

        "question" ->
          DecisionHelper.consult(:senior_architect, question, context["additional_context"])
      ...
      case result do
        ...
        {:error, :llm_disabled} ->
          {:ok, "AI consultation disabled. Enable with SENIOR_ARCHITECT_LLM_ENABLED=true"}
        {:error, reason} ->
  ...
  def execute_tool("session_consult", args) do
    ...
    case DecisionHelper.consult_session(:senior_architect, session_id, question, opts) do
      {:ok, result} ->
        response = format_session_response(result)
        {:ok, response}

      {:error, :llm_disabled} ->
        {:error, "LLM is disabled for Senior Architect. Enable with LLM_ENABLED=true or SENIOR_ARCHITECT_LLM_ENABLED=true"}
  ...
  defp get_engineering_metrics(time_range, _include_details) do
    ...
    technical_decisions =
      Repo.one(
        from d in Decision,
          where: d.initiator_role == "senior_architect" and d.inserted_at >= ^cutoff,
          select: count(d.id)
      ) || 0
    ...
    approved_decisions =
      Repo.one(
        from d in Decision,
          where: d.initiator_role == "senior_architect" and d.inserted_at >= ^cutoff and d.status == :approved,
          select: count(d.id)
      ) || 0
  ...
  defp validate_budget_authority(amount) do
    limit = Application.get_env(:senior_architect, :autonomous_budget_limit, 500_000)
  ...
  defp format_session_response(result) do
    model = EchoShared.LLM.Config.get_model(:senior_architect)

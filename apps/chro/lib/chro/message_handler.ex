defmodule Chro.MessageHandler do
  @moduledoc """
  Handles incoming messages from other agents via Redis pub/sub.

  Subscribes to:
  - messages:chro (direct messages)
  ...
  @impl true
  def init(_opts) do
    Logger.info("CHRO Message Handler started")

    # Subscribe to CHRO messages
    {:ok, _} = MessageBus.subscribe_to_role(:chro)
  ...
  defp handle_message("messages:chro", message, state) do
    Logger.info("CHRO received message: #{message["subject"]} from #{message["from"]}")
  ...
  defp handle_message("messages:all", message, state) do
    Logger.info("CHRO received broadcast: #{message["subject"]}")

    message_id = message["id"] || "unknown"

    if MapSet.member?(state.recent_broadcasts, message_id) do
      Logger.debug("CHRO already evaluated broadcast #{message_id}, skipping")
      state
    else
      case ParticipationEvaluator.should_participate?(:chro, message) do
        {:yes, confidence, type} ->
          Logger.info("CHRO participating as #{type} (confidence: #{confidence})")
          participate_in_task(message, type)

        {:no, reason} ->
          Logger.debug("CHRO declining participation: #{reason}")

        {:defer, _seconds} ->
          Logger.debug("CHRO evaluating with LLM (async)...")
      end
  ...
  defp handle_message("messages:leadership", message, state) do
    Logger.info("CHRO received leadership message: #{message["subject"]}")
  ...
  defp handle_escalation(message) do
    ...
    # TODO: Handle escalations requiring CHRO attention
  end
  ...
  defp handle_escalated_decision(event) do
    # Log escalated decision for CHRO review
    Logger.warning("""
    ESCALATED DECISION REQUIRES CHRO REVIEW
  ...
  defp participate_in_task(message, participation_type) do
    ...
    Logger.info("CHRO participating in task: #{subject} (as #{participation_type})")

    response_content = case participation_type do
      :lead -> "I'll lead this from an HR perspective. My expertise in people management will ensure organizational alignment."
      :assist -> "I can provide HR guidance on this. Let me know how I can support from a people perspective."
      :observe -> "I'll monitor this initiative to ensure it aligns with our people strategy and culture."
    end

    MessageBus.publish_message("chro", from, :response, "Re: #{subject}", %{
      content: response_content,
      participation_type: participation_type,
      timestamp: DateTime.utc_now() |> DateTime.to_iso8601()
    })

    MessageBus.store_message_in_db("chro", from, "response", "Re: #{subject}", %{
      content: response_content,
      participation_type: participation_type
    })

    Logger.info("✓ CHRO participation signal sent")
  end
end

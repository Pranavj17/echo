defmodule Cto do
  @moduledoc """
  CTO (Chief Technology Officer) MCP Server.

  The CTO is responsible for technology strategy, infrastructure architecture,
  ...
  def tools do
    [
      %{
        ...
        inputSchema: %{
          ...
          properties: %{
            ...
            rationale: %{
              type: "string",
              description: "CTO's rationale for the approval/rejection"
            },
            approved: %{
              type: "boolean",
      ...
      %{
        ...
        description: "Escalate a decision to CEO when it exceeds CTO authority",
        inputSchema: %{
          type: "object",
          properties: %{
            decision_id: %{
            ...
            recommendation: %{
              type: "string",
              description: "CTO's recommendation to CEO (optional)"
            }
          },
      ...
      %{
        name: "ai_consult",
        description: "Consult the CTO's AI advisor for technical insights, architecture analysis, and code generation",
        inputSchema: %{
          type: "object",
          properties: %{
            query_type: %{
              type: "string",
              enum: ["decision_analysis", "technical_question", "option_evaluation", "code_generation", "architecture_review"],
              description: "Type of AI consultation"
            },
            question: %{
              type: "string",
              description: "The question, task, or decision to analyze"
            },
            context: %{
              type: "object",
              description: "Additional context (options, requirements, constraints, language, etc.)"
            }
          },
  ...
  def execute_tool("approve_technical_proposal", args) do
    with {:ok, proposal_id} <- validate_required_string(args, "proposal_id"),
      ...
      Decision: #{if approved, do: "Approved", else: "Rejected"} by CTO
      Rationale: #{rationale}
  ...
  def execute_tool("allocate_engineering_budget", args) do
    with {:ok, recipient} <- validate_required_string(args, "recipient"),
      ...
      MessageBus.publish_message(
        ...
        :ceo,
        :notification,
        "CTO Budget Allocation: $#{format_number(amount)}",
        allocation
      )
      ...
      {:error, :budget_limit_exceeded} ->
        {:error, "Budget amount exceeds CTO autonomous authority ($500,000). Requires CEO approval."}
  ...
  def execute_tool("escalate_to_ceo", args) do
    with {:ok, decision_id} <- validate_required_string(args, "decision_id"),
      ...
      Urgency: #{urgency}
      Reason: #{reason}
      CTO Recommendation: #{args["recommendation"] || "None provided"}
    ...
    with {:ok, query_type} <- validate_required_string(args, "query_type"),
      ...
      result = case query_type do
        "decision_analysis" ->
          decision_context = Map.merge(context, %{
            decision_type: context["decision_type"] || "technical",
            context: question
          })
      ...
      case result do
        {:ok, response} ->
          ...
          ---
          Note: This is AI-generated technical advice. Validate architecture decisions with team and organizational standards.
          """}

        {:error, :llm_disabled} ->
          {:ok, "AI consultation is currently disabled for CTO role. Enable with CTO_LLM_ENABLED=true"}
  ...
  def execute_tool("session_consult", args) do
    ...
    case DecisionHelper.consult_session(:cto, session_id, question, opts) do
      {:ok, result} ->
        response = format_session_response(result)
        {:ok, response}

      {:error, :llm_disabled} ->
        {:error, "LLM is disabled for CTO. Enable with LLM_ENABLED=true or CTO_LLM_ENABLED=true"}
  ...
  defp get_engineering_metrics(time_range, _include_details) do
    ...
    technical_decisions =
      Repo.one(
        from d in Decision,
          where: d.initiator_role == "cto" and d.inserted_at >= ^cutoff,
          select: count(d.id)
      ) || 0
    ...
    approved_decisions =
      Repo.one(
        from d in Decision,
          where: d.initiator_role == "cto" and d.inserted_at >= ^cutoff and d.status == :approved,
          select: count(d.id)
      ) || 0
  ...
  defp validate_budget_authority(amount) do
    limit = Application.get_env(:cto, :autonomous_budget_limit, 500_000)
  ...
  defp consult_llm_for_decision(decision_context) do
    DecisionHelper.analyze_decision(:cto, decision_context)
  end

  defp consult_llm_for_evaluation(evaluation_context) do
    DecisionHelper.evaluate_options(:cto, evaluation_context)
  end

  defp consult_llm_for_code(task) do
    DecisionHelper.generate_implementation(:cto, task)
  end

  defp consult_llm_simple(question, context) do
    DecisionHelper.consult(:cto, question, context)
  end

  defp format_session_response(result) do
    model = EchoShared.LLM.Config.get_model(:cto)

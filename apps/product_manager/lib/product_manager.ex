defmodule ProductManager do
  @moduledoc """
  PRODUCT_MANAGER (Chief Technology Officer) MCP Server.

  The PRODUCT_MANAGER is responsible for technology strategy, infrastructure architecture,
  ...
  def tools do
    [
      %{
        ...
        inputSchema: %{
          ...
          properties: %{
            ...
            rationale: %{
              type: "string",
              description: "PRODUCT_MANAGER's rationale for the approval/rejection"
            },
            approved: %{
              type: "boolean",
      ...
      %{
        ...
        description: "Escalate a decision to CEO when it exceeds PRODUCT_MANAGER authority",
        inputSchema: %{
          type: "object",
          properties: %{
            decision_id: %{
            ...
            recommendation: %{
              type: "string",
              description: "PRODUCT_MANAGER's recommendation to CEO (optional)"
            }
          },
  ...
  def execute_tool("approve_technical_proposal", args) do
    with {:ok, proposal_id} <- validate_required_string(args, "proposal_id"),
      ...
      Decision: #{if approved, do: "Approved", else: "Rejected"} by PRODUCT_MANAGER
      Rationale: #{rationale}
  ...
  def execute_tool("allocate_engineering_budget", args) do
    with {:ok, recipient} <- validate_required_string(args, "recipient"),
      ...
      MessageBus.publish_message(
        ...
        :ceo,
        :notification,
        "PRODUCT_MANAGER Budget Allocation: $#{format_number(amount)}",
        allocation
      )
      ...
      {:error, :budget_limit_exceeded} ->
        {:error, "Budget amount exceeds PRODUCT_MANAGER autonomous authority ($500,000). Requires CEO approval."}
  ...
  def execute_tool("escalate_to_ceo", args) do
    with {:ok, decision_id} <- validate_required_string(args, "decision_id"),
      ...
      Urgency: #{urgency}
      Reason: #{reason}
      PRODUCT_MANAGER Recommendation: #{args["recommendation"] || "None provided"}
    ...
    with {:ok, query_type} <- validate_required_string(args, "query_type"),
      ...
      result = case query_type do
        "decision_analysis" ->
          ...
          DecisionHelper.analyze_decision(:product_manager, decision_context)
        ...
        "option_evaluation" ->
          ...
          DecisionHelper.evaluate_options(:product_manager, evaluation_context)

        "question" ->
          DecisionHelper.consult(:product_manager, question, context["additional_context"])
      ...
      case result do
        ...
        {:error, :llm_disabled} ->
          {:ok, "AI consultation disabled. Enable with PRODUCT_MANAGER_LLM_ENABLED=true"}
        {:error, reason} ->
  ...
  def execute_tool("session_consult", args) do
    ...
    case DecisionHelper.consult_session(:product_manager, session_id, question, opts) do
      {:ok, result} ->
        response = format_session_response(result)
        {:ok, response}

      {:error, :llm_disabled} ->
        {:error, "LLM is disabled for Product Manager. Enable with LLM_ENABLED=true or PRODUCT_MANAGER_LLM_ENABLED=true"}
  ...
  defp get_engineering_metrics(time_range, _include_details) do
    ...
    technical_decisions =
      Repo.one(
        from d in Decision,
          where: d.initiator_role == "product_manager" and d.inserted_at >= ^cutoff,
          select: count(d.id)
      ) || 0
    ...
    approved_decisions =
      Repo.one(
        from d in Decision,
          where: d.initiator_role == "product_manager" and d.inserted_at >= ^cutoff and d.status == :approved,
          select: count(d.id)
      ) || 0
  ...
  defp validate_budget_authority(amount) do
    limit = Application.get_env(:product_manager, :autonomous_budget_limit, 500_000)
  ...
  defp format_session_response(result) do
    model = EchoShared.LLM.Config.get_model(:product_manager)
